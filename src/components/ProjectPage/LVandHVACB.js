    ...
    render() {
        return (
            ...
            images={[
                ...
                {image: front,
                description:"Render of the accumulator PCB plate assembly, including the LV and HV Accumulator Control Boards."}]}
            description="For the 2020 car, we chose to change our BMS (Battery Management System) from an Elithion decentralized BMS to and Orion BMS 2, which is a centralized unit. This choice was made due to many issues encountered with packaging and troubleshooting of the decentralized unit's cell tap boards. The centralized unit offered simpler wiring at a cost of a much larger footprint. Due to the tight constraints within the battery pack, the board that interfaces between the BMS and the rest of the car's control systems had to be rethought from the previous year. To begin the design process, I worked with the vehicle controls captain to determine all of the necessary I/O for the BMS in order to determine what signals were needed for the shutdown circuitry mandated in the rules, which signals were needed for different vehicles states (charging vs discharging), etc. After documenting all the I/O, I began to create the Altium schematics for the board as well as designed the needed circuits on the board. This included a latching shutdown circuit that would disable the vehicle's tractive system if any BMS faults or faults in the insulation between HV and LV chassis ground were to occur. Circuitry was also implemented for the accumulator indicator circuit, which uses an isolated voltage sense IC paired with a comparator to light up an LED on the HV battery pack to indicate when high voltage is present at the terminals. Before proceeding to board layout, packaging needed to be sorted out with the lead battery designer in order to find out where the board could fit once the BMS, Insulation Monitoring Device, and current sensor were placed in the assembly. I developed an initial concept drawing for my planned placement which involved splitting the board into two separate PCBs that would go on either side of the BMS. I ended up calling these two boards the LV Accumulator Control Board and HV Accumulator Control Board. Through multiple iterations, we were able to make everything fit within the assembly. Renders courtesy of Jared Cox – Mechanical Team Captain, CAD assembly courtesy of Jessie Boudreau – Battery Pack Team Captain."
            />
        )
    }
}
