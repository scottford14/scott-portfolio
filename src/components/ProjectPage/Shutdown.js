...
export class Shutdown extends Component {
    render() {
        return (
            ...
            description="The shutdown circuit is a critical safety circuit that incorporates various interlocks to deactivate the vehicles tractive system in case of safety risk or system errors. The circuit I designed directly drives two contactors located in the HV battery pack that can disconnect the tractive system completely, isolating the driver and spectators from potential risks. Some of the circuits that I designed include the Brake System Plausibility Device, which is an analog circuit that checks for hard braking and positive current delivered to the motors, which prevents erroneous acceleration in the case of a stuck accelerator or CAN message corruption. The shutdown circuit would also be triggered if there is any faults with the BMS, both during charging of the battery pack and driving. Other interlocks include interlock loops on all HV connectors, to prevent unwanted HV exposure when disconnecting cables on the vehicle while the tractive system is active. Many other interlocks are present as well, which can be seen in the schematics above."
            />
        )
    }
}
