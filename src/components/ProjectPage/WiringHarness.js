...
export class WiringHarness extends Component {
    render() {
        return (
            ...
            description="The wiring within the vehicle required extensive design, given the large number of low voltage connections needed to implement all of the required circuitry for vehicle safety, vehicle controls, cooling, CAN Communication, driver cockpit buttons and warning lights, power delivery, and more. Extensive high voltage connections were also needed to supply power to all 4 wheel-mounted 3 phase motors, high voltage measuring points, and tractive system discharge circuitry. I designed the LV harness specifically to avoid noise introduced from other systems, ensuring that critical signals, such as CAN lines, and motor encoder cables, were properly shielded. LV signals were also routed as far as possible from the HV motor cables to reduce potential noise pollution as well. All HV cables were also shielded to further reduce EMI. After I completed the design, fellow member (Jared Cox) and I manufactured all the wires necessary, tested the harness for continuity, and installed it into the vehicle."
            />
        )
    }
}
